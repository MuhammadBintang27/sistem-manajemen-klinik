<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <span class="inline-flex items-center gap-2 px-4 py-2 bg-green-100 text-green-700 rounded-full text-sm font-semibold">
                <span class="inline-flex h-3 w-3 rounded-full bg-green-600"></span>
    ...
    <div class="space-y-8">
        ...
        <div class="rounded-2xl bg-gradient-to-r from-green-600 to-green-700 shadow-lg overflow-hidden text-white">
            <div class="px-8 py-12 sm:px-10">
                <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div>
                        <p class="text-sm font-semibold uppercase tracking-widest text-green-100">Dashboard Dokter</p>
                        <h1 class="mt-3 text-4xl font-bold">Pantau Pasien & Jadwal</h1>
                        <p class="mt-2 text-lg text-green-100">Ringkasan reservasi dan jadwal Anda dalam satu halaman.</p>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="rounded-2xl bg-white border border-green-100 shadow-md hover:shadow-lg transition-shadow p-8">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="mt-4 text-5xl font-bold text-green-600">{{ $reservasiCount }}</p>
        ...
        <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            <a href="{{ route('dokter.jadwal.index') }}" class="rounded-2xl bg-white border border-green-100 shadow-md hover:shadow-lg hover:border-green-300 transition-all p-6 text-center">
        ...
        <div class="rounded-2xl bg-white border border-green-100 shadow-md overflow-hidden">
            <div class="bg-gradient-to-r from-green-50 to-white border-b border-green-100 px-8 py-6">
                            ...
                            @foreach ($jadwals as $jadwal)
                                <tr class="hover:bg-green-50 transition-colors">
                                    ...
                                    <td class="px-8 py-4 whitespace-nowrap text-sm">
                                        <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold bg-green-100 text-green-800">{{ $jadwal->reservasi()->count() }}</span>
                                    </td>
                                    <td class="px-8 py-4 whitespace-nowrap text-sm">
                                        ...
                                        @else
                                            <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold bg-green-100 text-green-800">Tersedia</span>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
