<x-app-layout>
    ...
    <div class="space-y-6">
        ...
        <div class="rounded-2xl bg-white border border-secondary-100 shadow-md overflow-hidden">
            ...
            <div class="px-6 py-6">
                <form method="GET" action="{{ route('admin.rekam-medis.list-pasien') }}" class="flex gap-3">
                    <div class="flex-1">
                        <input 
                            type="text" 
                            name="search" 
                    ...
                    @if ($search)
                        <a href="{{ route('admin.rekam-medis.list-pasien') }}" class="px-6 py-2 bg-slate-300 hover:bg-slate-400 text-slate-900 font-medium rounded-lg transition-colors">
                            Reset
                        </a>
                    @endif
                </form>
        ...
        <div class="rounded-2xl bg-white border border-secondary-100 shadow-md overflow-hidden">
            ...
            @if ($pasiens->count() > 0)
                <div class="divide-y divide-slate-200">
                    @foreach ($pasiens as $pasien)
                        <a href="{{ route('admin.rekam-medis.pasien.show', $pasien) }}" class="block px-6 py-4 hover:bg-secondary-50 transition-colors border-b border-slate-100 last:border-b-0">
                            <div class="flex items-center justify-between">
                                <div class="flex-1">
            ...
            @else
                <div class="px-6 py-12 text-center">
                    ...
                    <p class="text-sm text-slate-500 mt-1">Belum ada rekam medis untuk pasien apapun</p>
                </div>
            @endif
        </div>
    </div>
